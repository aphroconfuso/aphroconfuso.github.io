	...
	if (list && browserTemplating && template) {
		bookmarksArray.sort((a, b) => new Date(b.dateTime) - new Date(a.dateTime)).forEach((bookmark, index) => {
			...
			if (title.startsWith('Djarju: ')) {
				({ mainTitle, derivedSequenceEpisodeTitle } = title.split(': '));
				storyType = 'djarju';
			}
			...
			if (sequenceEpisodeTitle) clone.querySelector("h3").textContent = derivedSequenceEpisodeTitle || sequenceEpisodeTitle;
